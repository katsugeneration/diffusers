...
class StableDiffusionImg2ImgPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        base_text_embeddings = text_embeddings.clone()
        ...
        if diffedit_noise_ratio is not None:
            ...
            unconditioned_text_embeddings = self.text_encoder(unconditioned_text_inputs.input_ids.to(self.device))[0]
            ...
            # Save mask image to local
            # mask_image = mask[None].repeat_interleave(3, dim=0).cpu().permute(1, 2, 0).numpy()
            # mask_image = self.numpy_to_pil(mask_image)
            # mask_image[0].save('mask.png')
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if mask is not None:
                latents = mask * latents + (1 - mask) * noisy_samples[t.item()]
